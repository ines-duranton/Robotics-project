...
#converts LineString object from lanelet2 to a MarkerArray
def LineString3d_to_marker(line_string, id):
    ...
    if "type" in line_string.attributes and line_string.attributes["type"] == "virtual":
        virtual(marker)
    elif "subtype" in line_string.attributes:
        subtype = line_string.attributes["subtype"]
...
def get_ros_param(node, param_name, default):
    """
    Accesses ROS Parameter of a node, if not found, uses default
    """
