...
#from jaxlib.xla_extension import jax_array #Depreciated
import jax
...
class Bicycle5D():
	...
	def get_jacobian_np(
		self, trajectory: jax_array, controls: jax_array
	) -> Tuple[np.ndarray, np.ndarray]:
		"""
	...
	def rollout_nominal_np(
			self, initial_state: np.ndarray, controls: jax_array
	) -> Tuple[jax_array, jax_array]:
		'''
	...
	def rollout_nominal_jax(
		...
		Returns:
			trajectory (jax_array): roll out trajectory along the nominal trajectory.
			ctrls_clip (jax_array): clipped control sequence
		'''
		n = controls.shape[1]
		@jax.jit
	...
	def get_jacobian_jax(
		...
		Args:
			trajectory (jax_array): trajectory along the nominal trajectory.
			controls (jax_array): controls along the trajectory.

		Returns:
			A (jax_array): the Jacobian of the dynamics w.r.t. the state.
			B (jax_array): the Jacobian of the dynamics w.r.t. the control.
		"""
	...
	def integrate_forward_jax(
		self, state: jax_array, control: jax_array
	) -> Tuple[jax_array, jax_array]:
		"""Clips the control and computes one-step time evolution of the system.

		Args:
			state (jax_array): [x, y, v, psi, delta].
			control (jax_array): [accel, omega].

		Returns:
			jax_array: next state. [5]
			jax_array: clipped control. [2]
		"""
	...
	###############################################################################
	@partial(jax.jit, static_argnums=(0,))
	def deriv(self, state: jax_array, control: jax_array) -> jax_array:
	...
	def _integrate_forward_dt(
		self, state: jax_array, control: jax_array, dt: float
	) -> jax_array:
		"""4th-order Runge-Kutta method.

		Args:
			state (jax_array): current state
			control (jax_array): current control
