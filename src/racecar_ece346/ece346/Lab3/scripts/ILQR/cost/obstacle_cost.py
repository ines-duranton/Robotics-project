...
class SingleObstacleCost(BaseCost):
    ...
    def get_running_cost(
			self, state: jax_array, ctrl: jax_array, ref: np.ndarray
	) -> float:
        '''
...
class ObstacleCost():
    ...
    def get_traj_cost(
			self, trajectory: jax_array, controls: jax_array, obs_refs: np.ndarray
	) -> float:
        '''
    ...
    def get_derivatives_jax(
            self, trajectory: jax_array, controls: jax_array, obs_refs: jax_array
    ) -> jax_array:
