...
class BaseCost(ABC):
	...
	def get_running_cost(
			self, state: jax_array, ctrl: jax_array, ref: jax_array
	) -> float:
		'''
	...
	@partial(jax.jit, static_argnums=(0,))
	def get_traj_cost(
			self, trajectory: jax_array, controls: jax_array, path_refs: jax_array
	) -> float:
		'''
...
@jax.jit
def exp_linear_cost(
		y: Union[float, jax_array], a: Union[float, jax_array] = 1,
		b: Union[float, jax_array] = 1
    ) -> Union[float, jax_array]:
	'''
