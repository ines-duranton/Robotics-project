...
class Cost():
    ...
    def get_traj_cost(
			self, trajectory: Union[np.ndarray, jax_array], 
            controls: Union[np.ndarray, jax_array], 
            path_refs: Union[np.ndarray, jax_array], 
            obs_refs: list = None
	) -> float:
        '''
    ...
    def get_derivatives_jax(
		...
		Input:
			...
			obs_refs: *Optional* (num_obstacle, (2, T)) List of obstacles. Default to None
		return:
			q: jax_array, (dim_x, T) jacobian of cost function w.r.t. states
            r: jax_array, (dim_u, T) jacobian of cost function w.r.t. controls
            Q: jax_array, (dim_x, dim_u, T) hessian of cost function w.r.t. states
            R: jax_array, (dim_u, dim_u, T) hessian of cost function w.r.t. controls
            H: jax_array, (dim_x, dim_u, T) hessian of cost function w.r.t. states and controls
		'''
